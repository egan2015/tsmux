...
block_fifo_t *block_FifoNew( void )
{
    ...
    pthread_cond_init( &p_fifo->wait_room,NULL );
    p_fifo->p_first = NULL;
...
void block_FifoRelease( block_fifo_t *p_fifo )
{
    ...
    pthread_cond_destroy( &p_fifo->wait_room );
...
void block_FifoEmpty( block_fifo_t *p_fifo )
{
    ...
    pthread_cond_broadcast( &p_fifo->wait_room );
...
void block_FifoPace (block_fifo_t *fifo, size_t max_depth, size_t max_size)
{
    ...
    while ((fifo->i_depth > max_depth) || (fifo->i_size > max_size))
    {
         ...
         pthread_cond_wait (&fifo->wait_room, &fifo->lock);
         //vlc_cleanup_pop ();
    }
...
void block_FifoWake( block_fifo_t *p_fifo )
{
    ...
    pthread_cond_broadcast( &p_fifo->wait );
    pthread_mutex_unlock( &p_fifo->lock );
}
